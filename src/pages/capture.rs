        ...
        if app.has_captured_image() {
            ui.with_layout(Layout::top_down(Align::Center), |ui| {
                ...
                if app.editor.cur_annotation.is_some() {
                    painter.add(app.editor.cur_annotation.as_mut()
                        .unwrap().render(scaling, to_screen));
                }

                let shapes: Vec<Shape> = app.editor.annotations.iter()
                    .map(|a| a.render(scaling, to_screen)).collect();
...
fn manage_input(app: &mut ScreenGrabber, input_response: Response, to_orginal: RectTransform) {
    if input_response.drag_started() {
        app.editor.cur_annotation = Some(Annotation::segment(to_orginal.transform_pos_clamped(
    ...
    if input_response.drag_released() {
        //TODO
        app.editor.annotations.push(app.editor.cur_annotation.clone().unwrap());
        app.editor.cur_annotation = None;
        return;
    }
    if app.editor.cur_annotation.is_some() {
        app.editor.cur_annotation.as_mut().unwrap().update(to_orginal.transform_pos_clamped(
            input_response.interact_pointer_pos().unwrap()
        ));
        return;
    }
